...
- **4.9.2-rc.22 (this rc, shipped):** Phase 1 Local v2. `WebGPUBackend.F64Mode` and `WebGLBackend.F64Mode` are now public settable properties initialized from `Options.F64Emulation` at construction. Codegen reads `Backend.F64Mode` (via the existing `EnableF64Emulation` / `UseOzakiF64Emulation` getters, which now read from the field) so flipping the mode at runtime affects subsequent kernel compiles. `WebGPUAccelerator.F64Mode` and `WebGLAccelerator.F64Mode` pass-throughs let the user flip the mode without recreating the accelerator. Lets a user that already created their accelerator promote it to strict-f64 without recreating the backend. Shader cache invalidation on mode-flip is NOT yet plumbed (cached kernels retain the mode they were compiled under) — future improvement; documented in the property doc-comments.
- **4.9.2-rc.23 (this rc, shipped):** Phase 1 P2P. `PeerCapabilities.F64ModesSupported` advertises Dekker/Ozaki on browser backends; `null` for native-f64 peers. `PeerCapabilities.SupportsStrictFloat64()` is the coordinator-side filter helper. `KernelDispatchRequest.F64Mode` carries the requested mode per dispatch; the worker's `HandleDispatchAsync` calls a new `ApplyRequestedF64Mode` helper that flips `WebGPUAccelerator.F64Mode` / `WebGLAccelerator.F64Mode` (using the rc.22 plumbing) before kernel compile. `P2PKernelSerializer.CreateDispatch` gains an optional `f64Mode` parameter so coordinator code can plumb through the AcceleratorRequirements flag without further surface changes. 7 new wire-protocol tests in `StrictFloat64WireTests` cover the round-trip + filter logic.
